...
public:
    ...
    void create_temporal_mass_matrix (const DoFHandler<1> &dof_handler_time, const FE_DGQ<1> &fe_time, FullMatrix<double> &temporal_mass_matrix) const; // Computes the temporal mass matrix M_ij = (phi_i, phi_j) where {phi_i} is the standard basis for the temporal dG space
...
{
FE_Q<dim> fe_space (space_degree); FE_DGQ<1> fe_time (time_degree); FESystem<dim> fe (fe_space, time_degree + 1);
...
create_temporal_mass_matrix (dof_handler_time, fe_time, temporal_mass_matrix); 
...
template <int dim> void dGcGblowup<dim>::create_temporal_mass_matrix (const DoFHandler<1> &dof_handler_time, const FE_DGQ<1> &fe_time, FullMatrix<double> &temporal_mass_matrix) const
{
const unsigned int no_of_time_dofs = dof_handler_time.n_dofs();
const QGauss<1> quadrature_formula_time (no_of_time_dofs);
...
typename DoFHandler<1>::active_cell_iterator time_cell = dof_handler_time.begin_active (); fe_values_time.reinit (time_cell);
    ...
    for (unsigned int r = 0; r < no_of_time_dofs; ++r)
        for (unsigned int s = 0; s < r + 1; ++s)
        {
            for (unsigned int q_time = 0; q_time < no_of_time_dofs; ++q_time)
...
{
FE_Q<dim> fe_space (space_degree);
...
const Functions::FEFieldFunction<dim> old_solution_plus_function (old_dof_handler_space, old_solution.block(old_time_degree));
...
{
switch (time_degree) {case 0: temporary_solution = old_solution.block(old_time_degree); break; default: extend_to_constant_in_time_function (old_solution.block(old_time_degree), temporary_solution);}
}
else // If the mesh has changed, we do as above but must first interpolate to the current finite element space
{
VectorTools::interpolate_to_different_mesh (old_dof_handler_space, old_solution.block(old_time_degree), dof_handler_space, solution.block(0));
...
unsigned int iteration_number = 1; double residual = 0; const double max = old_solution.block(old_time_degree).linfty_norm();
...
{
FE_Q<dim> fe_space (space_degree); FE_DGQ<1> fe_time (time_degree);
...
old_old_solution_plus = old_solution.block(old_time_degree);
}
else
{
old_old_solution_plus = old_solution.block(old_time_degree); 
...
if (time_degree != old_time_degree || mesh_change == true)
{
if (time_degree != old_time_degree) {old_time_degree = time_degree;}
if (mesh_change == true) {old_triangulation_space.clear (); old_triangulation_space.copy_triangulation (triangulation_space); old_dof_handler_space.distribute_dofs (fe_space);}
...
std::ofstream solution_values_file ("solution_values.txt"); solution_values_file << std::setprecision (9) << old_solution.block(time_degree).linfty_norm () << std::setprecision (6); solution_values_file.close ();
...
std::ofstream solution_values_file ("solution_values.txt", std::ios::app); solution_values_file << std::endl << std::setprecision (9) << solution.block(time_degree).linfty_norm () << std::setprecision (6); solution_values_file.close ();
...
{
FE_Q<dim> fe_space (space_degree); FESystem<dim> fe (fe_space, time_degree + 1);
...
{
FE_Q<dim> fe_space (space_degree); FE_DGQ<1> fe_time (time_degree); FE_DGQ<1> old_fe_time (old_time_degree);
...
FullMatrix<double> old_temporal_mass_matrix_inv (old_time_degree + 1, old_time_degree + 1);

Vector<double> L2_projection_rhs (time_degree + 1); Vector<double> old_L2_projection_rhs (old_time_degree + 1); Vector<double> L2_projection_f (time_degree + 1); std::vector<double> old_old_solution_plus_values (no_q_space);
std::vector<double> Q_values (no_q_time); std::vector<double> Q_derivative_values (no_q_time); std::vector<double> Q_second_derivative_values (no_q_time); std::vector<double> old_Q_derivative_values (no_q_time);
...
create_temporal_mass_matrix (old_dof_handler_time, old_fe_time, old_temporal_mass_matrix_inv); old_temporal_mass_matrix_inv.gauss_jordan ();
    ...
    for (unsigned int q_time = 0; q_time < no_q_time; ++q_time)
    {
    compute_Q_values (old_time_degree, 2*quadrature_formula_time.point(q_time)(0) - 1, Q_values[q_time], old_Q_derivative_values[q_time], Q_second_derivative_values[q_time]);
...
const double h_min = GridTools::minimal_cell_diameter (triangulation_space); const double ell_h = log(2 + 1/h_min);
    ...
    recon_sol_at_q_time = solution.block(0); recon_sol_at_q_time.add(-1, old_solution.block(old_time_degree)); recon_deriv_at_q_time = recon_sol_at_q_time; 
    ...
    typename DoFHandler<dim>::active_cell_iterator cell = dof_handler_space.begin_active (), final_cell = dof_handler_space.end ();
        ...
        if (timestep_number > 1) {fe_values_space.get_function_values (old_old_solution_plus, old_old_solution_plus_values);} else {initialvalueslaplacian<dim>().value_list (fe_values_space.get_quadrature_points(), old_old_solution_plus_values);}
            ...
            {
            jump += (1/dt_old)*old_Q_derivative_values[no_q_time - 1]*(old_solution_at_q_pt[0] - old_old_solution_plus_values[q_space]);

            switch (old_time_degree)
            {
            case 0: jump -= old_solution_at_q_pt[q_time]*old_solution_at_q_pt[q_time]; break;        
            default: old_L2_projection_rhs = 0;
                ...
                const double nonlinearity_value = old_solution_at_q_pt[q_time]*old_solution_at_q_pt[q_time]*old_fe_values_time.JxW(q_time);
                    ...
                    for (unsigned int r = 0; r < old_time_degree + 1; ++r)
                    old_L2_projection_rhs(r) += nonlinearity_value*old_fe_values_time.shape_value(r,q_time);
                }

                for (unsigned int r = 0; r < old_time_degree + 1; ++r)
                {
                jump -= old_temporal_mass_matrix_inv(old_time_degree, r)*old_L2_projection_rhs(r);
...
GridGenerator::create_union_triangulation (old_triangulation_space, old_old_triangulation_space, intermediate_triangulation);
GridGenerator::create_union_triangulation (triangulation_space, intermediate_triangulation, union_triangulation);
}

DoFHandler<dim> union_dof_handler_space (union_triangulation); union_dof_handler_space.distribute_dofs (fe_space);

const unsigned int no_of_union_space_dofs = union_dof_handler_space.n_dofs ();

BlockVector<double> solution_union (time_degree + 1); BlockVector<double> old_solution_union (old_time_degree + 1);
...
Vector<double> union_refinement_vector; if (output_refinement_vector == true) {union_refinement_vector.reinit (union_triangulation.n_active_cells());}
...
const double h_min = GridTools::minimal_cell_diameter (union_triangulation); const double ell_h = log(2 + 1/h_min);

    for (unsigned int q_time = 0; q_time < no_q_time; ++q_time)
    {
    double space_est_at_q_time = 0; double deriv_space_est_at_q_time = 0;  

    Vector<double> recon_sol_at_q_time (no_of_union_space_dofs); Vector<double> recon_deriv_at_q_time (no_of_union_space_dofs);

    recon_sol_at_q_time = solution_union.block(0); recon_sol_at_q_time.add(-1, old_solution_union.block(old_time_degree)); recon_deriv_at_q_time = recon_sol_at_q_time; 
    recon_sol_at_q_time *= Q_values[q_time]; recon_deriv_at_q_time *= (1/dt)*Q_derivative_values[q_time]; 

        for (unsigned int i = 0; i < no_of_union_space_dofs; ++i)
	        for (unsigned int r = 0; r < time_degree + 1; ++r)
            {
            recon_sol_at_q_time(i) += solution_union.block(r)(i)*fe_values_time.shape_value(r,q_time);
            if (time_degree > 0) {recon_deriv_at_q_time(i) += solution_union.block(r)(i)*fe_values_time.shape_grad(r,q_time)[0];}
	        }

    const double recon_sol_at_q_time_max = recon_sol_at_q_time.linfty_norm ();

    typename DoFHandler<dim>::active_cell_iterator cell = union_dof_handler_space.begin_active (), final_cell = union_dof_handler_space.end ();
        ...
        fe_values_space.get_function_values (old_old_solution_plus_union, old_old_solution_plus_values);
            ...
            std::vector<double> solution_at_q_pt (no_q_time); std::vector<double> old_solution_at_q_pt (no_q_time);
                ...
                for (unsigned int q_time = 0; q_time < no_q_time; ++q_time)
                    ...
                    {
                        for (unsigned int r = 0; r < time_degree + 1; ++r)
                        solution_at_q_pt[q_time] += solution_union.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*fe_values_time.shape_value(r,q_time);

                        for (unsigned int r = 0; r < old_time_degree + 1; ++r)
                        old_solution_at_q_pt[q_time] += old_solution_union.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*old_fe_values_time.shape_value(r,q_time);
                    }
                ...
                {
                    for (unsigned int r = 0; r < time_degree + 1; ++r)
                    deriv_space_est_at_q_pt -= solution_union.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*fe_values_time.shape_hessian(r,q_time)[0][0];
                }              
                }

            if (time_degree > 0) {deriv_space_est_at_q_pt -= (1/dt)*(1/dt)*Q_second_derivative_values[q_time]*(solution_at_q_pt[0] - old_solution_at_q_pt[no_q_time - 1]);}

            jump = (1/dt)*Q_derivative_values[0]*(old_solution_at_q_pt[no_q_time - 1] - solution_at_q_pt[0]) + (1/dt_old)*old_Q_derivative_values[no_q_time - 1]*(old_solution_at_q_pt[0] - old_old_solution_plus_values[q_space]);
            ...
            jump += L2_projection_f(0);
                ...
                deriv_space_est_at_q_pt += L2_projection_f(r)*fe_values_time.shape_grad(r,q_time)[0];
                    ...
                    for (unsigned int i = 0; i < dofs_per_cell; ++i)
                    jump -= solution_union.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*fe_values_time.shape_grad(r,0)[0];
                }
            }
            ...
            default: old_L2_projection_rhs = 0;
                ...
                jump -= old_temporal_mass_matrix_inv(old_time_degree, r)*old_L2_projection_rhs(r);
                    ...
                    for (unsigned int i = 0; i < dofs_per_cell; ++i)
                    jump += old_solution_union.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*old_fe_values_time.shape_grad(r,no_q_time - 1)[0];
                }
            }
        ...
        if (output_refinement_vector == true) {union_refinement_vector(cell_no) += (space_est_cell*(2*recon_sol_at_q_time_max + space_est_cell) + deriv_space_est_cell)*fe_values_time.JxW(q_time);}
...
auto cell_pair = cell_list.begin(); auto final_cell_pair = cell_list.end();
    ...
    {
    refinement_vector(cell_pair->first->active_cell_index()) = union_refinement_vector(cell_pair->second->active_cell_index());
    }
    else
    {
    auto active_subcells = GridTools::get_active_child_cells<Triangulation<dim>> (cell_pair->second);
        ...
        refinement_vector(cell_pair->first->active_cell_index()) = fmax(refinement_vector(cell_pair->first->active_cell_index()), union_refinement_vector(active_subcells[subcell]->active_cell_index()));
    }
    }
}
}
...
Vector<double> old_L2_projection_rhs (old_time_degree + 1);
...
std::vector<double> old_Q_derivative_values (no_q_time);
std::vector<types::global_dof_index> local_dof_indices (dofs_per_cell);
...
fe_values_time.reinit (time_cell); old_fe_values_time.reinit (old_time_cell);
    ...
    for (unsigned int q_time = 0; q_time < no_q_time; ++q_time)
    {
    compute_Q_values (old_time_degree, 2*quadrature_formula_time.point(q_time)(0) - 1, Q_values[q_time], old_Q_derivative_values[q_time], time_est);
...
typename DoFHandler<dim>::active_cell_iterator cell = dof_handler_space.begin_active (), final_cell = dof_handler_space.end ();
    ...
    for (; cell != final_cell; ++cell)
    {
    fe_values_space.reinit (cell);

    cell->get_dof_indices (local_dof_indices);

    solution_values = 0; old_solution_values = 0;
        ...
        for (unsigned int q_space = 0; q_space < no_q_space; ++q_space)
            for (unsigned int q_time = 0; q_time < no_q_time; ++q_time)
                ...
                {
                    for (unsigned int r = 0; r < time_degree + 1; ++r)
                    solution_values(q_space + q_time*no_q_space) += solution.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*fe_values_time.shape_value(r,q_time);

                if (timestep_number > 1)
                {
                    for (unsigned int r = 0; r < old_time_degree + 1; ++r)
                    old_solution_values(q_space + q_time*no_q_space) += old_solution.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*old_fe_values_time.shape_value(r,q_time);
                }
                }
    ...
    if (timestep_number > 1) {fe_values_space.get_function_values (old_old_solution_plus, old_old_solution_plus_values);}
        ...
        default: L2_projection_f(0) = 0; old_L2_projection_rhs = 0; solution_time_derivative_value = 0;
            ...
            nonlinearity_value = old_solution_values(q_space + q_time*no_q_space)*old_solution_values(q_space + q_time*no_q_space)*old_fe_values_time.JxW(q_time);
	            ...
	            for (unsigned int i = 0; i < old_time_degree + 1; ++i)
	            old_L2_projection_rhs(i) += nonlinearity_value*old_fe_values_time.shape_value(i, q_time);
            }

            for (unsigned int i = 0; i < old_time_degree + 1; ++i)
            L2_projection_f(0) += old_temporal_mass_matrix_inv(old_time_degree, i)*old_L2_projection_rhs(i);
...
DoFHandler<dim> dof_handler_space_union (union_triangulation); dof_handler_space_union.distribute_dofs (fe_space);
...
VectorTools::interpolate_to_different_mesh (old_old_dof_handler_space, old_old_solution_plus, dof_handler_space_union, old_old_solution_plus_union);

typename DoFHandler<dim>::active_cell_iterator union_cell = dof_handler_space_union.begin_active (), final_union_cell = dof_handler_space_union.end ();

    // Loop over all cells, compute the Linfty norm of the temporal residual of the solution at each temporal quadrature point and store it in estimator_values

    for (; union_cell != final_union_cell; ++union_cell)
    {
    fe_values_space.reinit (union_cell);
    ...
    solution_values = 0; old_solution_values = 0;
        ...
        for (unsigned int q_space = 0; q_space < no_q_space; ++q_space)
            for (unsigned int q_time = 0; q_time < no_q_time; ++q_time)
                ...
                {
                    for (unsigned int r = 0; r < time_degree + 1; ++r)
                    solution_values(q_space + q_time*no_q_space) += solution_union.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*fe_values_time.shape_value(r,q_time);

                    for (unsigned int r = 0; r < old_time_degree + 1; ++r)
                    old_solution_values(q_space + q_time*no_q_space) += old_solution_union.block(r)(local_dof_indices[i])*fe_values_space.shape_value(i,q_space)*old_fe_values_time.shape_value(r,q_time);
                }
...
deallog << "r: " << r << std::endl << std::endl; // Output the value of the scaling parameter r
}
}
...
deallog << std::endl << "~~Setting up the initial mesh and timestep length on the first timestep~~" << std::endl;
    ...
    {
    dt *= 0.5; dt_old = dt; dt_init = dt; triangulation_time.clear(); GridGenerator::hyper_cube (triangulation_time, 0, dt); old_triangulation_time.clear(); old_triangulation_time.copy_triangulation (triangulation_time); mesh_change = true;
    }
    ...
    if (time_degree != old_time_degree) {deallog << "The polynomial degree in time has changed. ";}

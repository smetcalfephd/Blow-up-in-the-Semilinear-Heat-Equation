...
private:
	...
	Triangulation<dim> triangulation_space; Triangulation<dim> old_triangulation_space; // The current mesh, the mesh from the previous timestep and the mesh from the previous previous timestep
	...
	DoFHandler<dim> dof_handler_space; DoFHandler<dim> old_dof_handler_space; 
	DoFHandler<1> dof_handler_time; DoFHandler<1> old_dof_handler_time; 
	DoFHandler<dim> dof_handler;

    FE_Q<dim> fe_space; FE_Q<dim> old_fe_space;
...
template <int dim> dGcGblowup<dim>::dGcGblowup ()
                :
				dof_handler_space (triangulation_space), old_dof_handler_space (old_triangulation_space),
				dof_handler_time (triangulation_time), old_dof_handler_time (old_triangulation_time),
				dof_handler (triangulation_space),
				fe_space (space_degree), old_fe_space (space_degree),
...
{
dof_handler_space.distribute_dofs (fe_space); old_dof_handler_space.distribute_dofs (old_fe_space);
...
solution.reinit (time_degree + 1);  old_solution.reinit (time_degree + 1);
    ...
    {
    solution.block(r).reinit (no_of_space_dofs); old_solution.block(r).reinit (no_of_space_dofs);
    }

solution.collect_sizes (); old_solution.collect_sizes ();

old_old_solution_plus.reinit (no_of_space_dofs);
...
DynamicSparsityPattern dsp_space (no_of_space_dofs);
DoFTools::make_sparsity_pattern (dof_handler_space, dsp_space, spatial_constraints, false);
spatial_sparsity_pattern.copy_from (dsp_space);

Vector<double> right_hand_side (no_of_space_dofs);

SparseMatrix<double> mass_matrix; mass_matrix.reinit (spatial_sparsity_pattern);
MatrixCreator::create_mass_matrix (dof_handler_space, QGauss<dim>(space_degree + 1), mass_matrix, (const Function<dim,double> *const)nullptr, spatial_constraints);

SparseILU<double> preconditioner; preconditioner.initialize (mass_matrix);
...
SolverBicgstab<> solver (solver_control, data);
    ...
    for (unsigned int r = 0; r < time_degree + 1; ++r)
    {
    Functions::FEFieldFunction<dim> old_solution_as_function (old_dof_handler_space, old_solution.block(r));

    VectorTools::create_right_hand_side (dof_handler_space, QGauss<dim>(space_degree + 1), old_solution_as_function, right_hand_side, spatial_constraints);
    solution.block(r).reinit (no_of_space_dofs); old_solution.block(r).reinit (dof_handler_space.n_dofs());
    
    solver.solve (mass_matrix, old_solution.block(r), right_hand_side, preconditioner);
    spatial_constraints.distribute (old_solution.block(r));
    }

solution.collect_sizes (); old_solution.collect_sizes ();

Functions::FEFieldFunction<dim> old_old_solution_plus_as_function (old_dof_handler_space, old_old_solution_plus);

VectorTools::create_right_hand_side (dof_handler_space, QGauss<dim>(space_degree + 1), old_old_solution_plus_as_function, right_hand_side, spatial_constraints);
old_old_solution_plus.reinit (no_of_space_dofs);

solver.solve (mass_matrix, old_old_solution_plus, right_hand_side, preconditioner);
spatial_constraints.distribute (old_old_solution_plus);
...
// Extend the numerical solution at final time on the previous interval to a constant-in-time function for use as an initial guess in the nonlinear iteration
...
unsigned int iteration_number = 1; double residual = 0; double max = old_solution.block(time_degree).linfty_norm();
    ...
    double cell_size = 0; double previous_cell_size = 0; double cell_size_check = 0; right_hand_side = 0;
        ...
        if (iteration_number == 1) {fe_values_space.get_function_values (old_solution.block(time_degree), old_solution_plus_values);}
...
mesh_change = false; // Reset the mesh change parameter in preparation for the next time step
}
